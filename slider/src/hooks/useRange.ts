...
const useRange = ({
  ...
  minRef.current = React.useMemo(
    () => Math.max(minimumValue, minProp),
    [minProp, minimumValue],
  );
  maxRef.current = React.useMemo(
    () => Math.min(maximumValue, maxProp),
  ...
  const onMinChange = useEvent((min: number) => {
    minRef.current = min;
    onValueChange?.(
      [min, maxRef.current].sort((a, b) => a - b) as [number, number],
    );
  });
  const onMaxChange = useEvent((max: number) => {
    maxRef.current = max;
    onValueChange?.(
      [minRef.current, max].sort((a, b) => a - b) as [number, number],
    );
  });
  ...
  const range = React.useMemo(
    () => [minValue, maxValue].sort((a, b) => a - b) as [number, number],
    [maxValue, minValue],
  );

  const currentThumb = React.useRef<'min' | 'max' | undefined>(undefined);

  // Method to update the lower or higher bound according to which one is the closest
  const updateClosestValue = useEvent(
    (value: number, state: 'press' | 'release' | 'drag') => {
      ...
      if (currentThumb.current && !crossingAllowed)
        ...
        isMinClosest =
          ...
          (minValue === maxValue && value < minValue);
