...
const Thumb = ({
  ...
  const thumbContainerStyle = React.useMemo<RN.StyleProp<RN.ViewStyle>>(
    () => getThumbContainerStyle(CustomThumb ? undefined : size),
    [CustomThumb, size],
  );
  const thumbViewStyle = React.useMemo<RN.StyleProp<RN.ImageStyle>>(
    () => [getThumbStyle(size, color), style as RN.ImageStyle],
    [style, size, color],
  );

  // Accessibility actions
  const accessibilityActions = useEvent(
    (event: RN.AccessibilityActionEvent) => {
  ...
  const handleAccessibilityKeys = useEvent(
    (event: RN.NativeSyntheticEvent<RN.TextInputKeyPressEventData>) => {
  ...
  const accessibilityValues = React.useMemo(() => {
    ...
    const isDecimal =
      ...
      maximumValue % 1 !== 0 ||
      step % 1 !== 0 ||
      (step === 0 && ((maximumValue - minimumValue) / 10) % 1 !== 0);
    return isDecimal
      ? {
          min: 0,
          max: 100,
          now: Math.round((value / (maximumValue - minimumValue || 1)) * 100),
        }
  ...
  return (
    <RN.View style={stylesheet.container}>
      <RN.View
        ...
        {thumbImage ? (
          <RN.Image source={thumbImage} style={thumbViewStyle} />
        ) : CustomThumb ? (
          <CustomThumb value={value} thumb={thumb} />
        ) : (
