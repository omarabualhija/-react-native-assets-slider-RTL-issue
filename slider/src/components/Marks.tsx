...
export type CustomMarkType = React.ComponentType<{
  value: number;
  active: boolean;
}>;
...
const Marks = ({
  ...
  const onLayoutUpdateMarks = useEvent<
    Exclude<ViewProps['onLayout'], undefined>
  >(event => {
  ...
  const marks = React.useMemo<any | null>(() => {
    if (!CustomMark) return null;
    const markCount =
      Math.round((maximumValue - minimumValue) / (step || 1)) + 1;
    return Array(markCount)
      .fill(0)
      .map((_, index) => {
        const markValue = round(index * step + minimumValue);
        const advance =
          (((vertical ? sliderHeight : sliderWidth) - THUMB_SIZE) *
            (markValue - minimumValue)) /
            (maximumValue - minimumValue || 1) +
          THUMB_SIZE / 2;
        const padding = (vertical ? sliderWidth : sliderHeight) / 2;
        const x = inverted
          ? (vertical ? sliderHeight : sliderWidth) - advance
          : advance;
        const y = padding;
        return (
          <Mark
  ...
  return (
    <View style={styleSheet.container} onLayout={onLayoutUpdateMarks}>
      {marks}
    </View>
  );
};
