...
const ResponderView = React.forwardRef<RN.View, Props>((props, ref) => {
  ...
  const isVertical = React.useMemo(
    () =>
      vertical ||
      (style &&
        (RN.StyleSheet.flatten(style).flexDirection || '').startsWith(
          'column',
        )),
    [vertical, style],
  );
  ...
  React.useEffect(() => {
    ...
    fallbackRef.current?.measure(
      (_x, _y, _width, _height, pageX = 0, pageY = 0) =>
        (originPageLocation.current = {pageX, pageY}),
    );
  ...
  const eventToValue = useEvent((event: RN.GestureResponderEvent) => {
    ...
    const x =
      RN.Platform.OS === 'web'
        ? locationX
        : pageX - originPageLocation.current.pageX;
    const y =
      RN.Platform.OS === 'web'
        ? locationY
        : pageY - originPageLocation.current.pageY;

    let offset = isVertical ? y : x;
    const size = containerSize.current?.[isVertical ? 'height' : 'width'] || 1;

    // RTL adjustment for horizontal sliders
    if (!isVertical && RN.I18nManager.isRTL) {
      offset = size - offset;
    }
  ...
  return (
    <RN.View
      ...
      <SliderView vertical={vertical} inverted={inverted}>
        {props.children}
      </SliderView>
    </RN.View>
  );
