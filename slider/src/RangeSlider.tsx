...
const RangeSlider = React.forwardRef<RN.View, RangeSliderProps>(
  (props: RangeSliderProps, forwardedRef) => {
    const {
      minimumValue = 0,
      step = 0,
      minimumRange = step,
      ...
      () => [
        ...
        maxProp ??
          Math.max(
            ...
            (props.minimumValue || 0) + (props.minimumRange || props.step || 0),
          ),
      ],
      [
        maxProp,
    ...
    const {updateClosestValue, updateMaxValue, updateMinValue, range, canMove} =
      useRange({
        minimumRange,
        minimumValue,
        maximumValue,
    ...
    const [minStyle, midStyle, maxStyle] = React.useMemo(
      () =>
        [minTrackStyle, midTrackStyle, maxTrackStyle].map(style =>
          style && trackStyle ? [style, trackStyle] : style || trackStyle,
        ),
    ...
    const thumbProps = {
      ...
      style: thumbStyle,
      size: thumbSize,
      CustomThumb: CustomThumb as React.ComponentType<{
        value: number;
        thumb?: 'min' | 'max';
      }>,
      thumbImage,
